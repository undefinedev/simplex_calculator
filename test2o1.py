...
class SimplexSolutionWindow(QWidget):
    def __init__(self):
        ...
        # Optional: Adjust selection behavior
        self.table.setSelectionBehavior(QTableWidget.SelectItems)
        self.table.setSelectionMode(QTableWidget.SingleSelection)

        # Add navigation buttons for iterations (optional for future steps)
    ...
    def add_step(self, tableau_df):
        """
        Add a new step by setting the tableau data in the QTableWidget.
        :param tableau_df: pandas DataFrame representing the tableau
        """
        self.steps.append(tableau_df)
        if len(self.steps) == 1:
            # If this is the first step, display it
            self.current_step_index = 0
            self.display_tableau(self.steps[0])
            self.next_button.setEnabled(True)
        else:
            # For subsequent steps, append the tableau or handle as needed
            pass  # Can be expanded for step-by-step navigation

    def display_tableau(self, tableau_df):
        """
        Display the given tableau DataFrame in the QTableWidget.
        :param tableau_df: pandas DataFrame representing the tableau
        """
        ...
        # Set headers
        self.table.setHorizontalHeaderLabels(tableau_df.columns.tolist())
        self.table.setVerticalHeaderLabels(tableau_df.index.tolist())
    ...
    def highlight_pivot(self, row_label, col_label):
        """
        Highlight the pivot element in the tableau.
        :param row_label: The row header label of the pivot element
        :param col_label: The column header label of the pivot element
        """
        # Find the row index
        row_indices = self.table.findItems(row_label, Qt.MatchExactly)
        if not row_indices:
            return
        row = row_indices[0].row()

        # Find the column index
        col_indices = self.table.findItems(col_label, Qt.MatchExactly)
        if not col_indices:
            return
        col = col_indices[0].column()

        # Clear previous highlights
        self.clear_highlights()

        # Highlight the pivot cell
        pivot_item = self.table.item(row, col)
        if pivot_item:
            pivot_item.setBackground(Qt.yellow)  # Choose a highlight color
    ...
    def prev_step(self):
        if self.current_step_index > 0:
            self.current_step_index -= 1
            self.display_tableau(self.steps[self.current_step_index])
            self.next_button.setEnabled(True)
        if self.current_step_index == 0:
...
class SimplexCalculator(QWidget):
    ...
    def initUI(self):
        ...
        self.num_vars_spin.setMaximum(20)
        ...
        self.num_constraints_spin.setMaximum(20)
    ...
    def solve(self):
        try:
            ...
            for i in range(num_constraints):
                ...
                if len(qlineedits) < expected_qlineedits or len(qcomboboxes) < 1:
                    QMessageBox.warning(
                        self,
                        "Input Error",
                        f"Constraint {i + 1} имеет неверные поля ввода."
                    )
                    return
                ...
                if relation == "=":
                    ...
                    if len(valid_candidates) == 1:
                        ...
                        QMessageBox.information(
                            self,
                            "Базовая переменная выбрана автоматически",
                            f"Для ограничения {idx + 1} было выбрано {basic_var} как базовая переменная."
                        )
                    else:
                        # No suitable basic variable found, prompt to add a new one
                        reply = QMessageBox.question(
                            self,
                            "Отсутствует базовая переменная",
                            f"В ограничении {idx + 1} нет переменной с коэффициентом 1 или -1, которая уникальна.\n"
                            "Хотите автоматически добавить новую базовую переменную?",
                        ...
                        if reply == QMessageBox.Yes:
                            ...
                            QMessageBox.information(
                                self,
                                "Добавлена новая базовая переменная",
                                f"Базовая переменная {new_basic_var} была автоматически добавлена для ограничения {idx + 1}."
                            )
                        else:
                            ...
                            QMessageBox.information(
                                ...
                                "Пожалуйста, отредактируйте ограничения, чтобы каждая базовая переменная была уникальной и имела коэффициент 1 или -1."
                            )
                            return
                elif relation in ["≤", "≥"]:
                    ...
                    QMessageBox.warning(
                        self,
                        "Input Error",
                        f"Constraint {idx + 1} имеет недопустимое отношение."
                    )
                    return
            ...
            if len(final_decision_vars) < len(decision_vars):
                QMessageBox.information(
                    self,
                    "Исключение базовых переменных",
                    "Некоторые переменные были автоматически исключены из целевой функции, так как они являются базовыми переменными."
                )
            ...
            self.solution_window.add_step(df)
